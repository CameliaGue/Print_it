...
// Initialisation de l'index du slide actuel (on rapporte tous les élements)

let currentSlideIndex = 0;

function updateSlide() {
	console.log("updateSlide appelé");
	// Affichage de l'index du slide actuel
	console.log("Index actuel :", currentSlideIndex);
	// Vérification de l'index du slide actuel 
	const slide = slides[currentSlideIndex];
	console.log("Slide actuel :", slide);
	// Mise à jour de l'image du slide
...
function updateDots() {
	...
	// Suppression des anciens dots plus simple pour refaire les dots
	dotsContainer.innerHTML = "";
	slides.forEach((slide, index) => {
		console.log("Création dot :", index);
		// Création d'un élément span pour le dot
		const dot = document.createElement("span");
		dot.classList.add("dot");
		if (index === currentSlideIndex) {
			console.log("Dot sélectionné :", index);
			// Ajout de la classe pour le dot sélectionné
...
leftArrow.addEventListener("click", () => {
	...
	console.log("Nouvel index :", currentSlideIndex); 
	// Mise à jour de l'affichage du slide
	updateSlide();
});
